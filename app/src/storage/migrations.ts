...
import { createDefaultDayContext, type DayDraft } from '../domain/days';
import { isValidDateKey } from '../domain/dates';
import type { Meal, MealItem } from '../domain/meals';
import {
  DEFAULT_PROFILE,
  ...
  return {
    ...
    context: {
      ...
      trainingIntensity: stringOr(
        context.trainingIntensity,
        ['easy', 'normal', 'hard'] as const,
        defaultContext.trainingIntensity,
      ),
      actualSteps: typeof context.actualSteps === 'number' ? context.actualSteps : undefined,
      trainingMinutes:
        typeof context.trainingMinutes === 'number' ? context.trainingMinutes : undefined,
    },
    meals,
  };
}
